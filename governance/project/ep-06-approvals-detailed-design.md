# EP-06: Detailed Design — Security, External Access & Delegation

**Version:** 1.0  
**Date:** 2026-05-14  
**Epic:** EP-06 (Post-MVP)  
**User Stories:** US-017 to US-022  
...
**FS-09** implements adaptive authentication where:
...
| **Tenant Risk Level** | 0-30 | 0.20 | Tenant categorized as "high-risk" |
...
**I want:** Adaptive MFA rules to require verification on risky access  
**So that:** Security posture improves without uniform friction
...
| **TOTP** | Time-based OTP (Google Authenticator, Authy) | 🔒🔒 (Medium) | ⭐⭐ (Good) | Authenticator app |
...
**FS-14** allows administrators to delegate management authority to others with controlled boundaries:
...
└─────────────────────────────────────────────────────────┘
              ...
              ┌─ [PENDING_APPROVAL] ──┐
              │                       │
    (If requires approval)      (If doesn't require)
              │                       │
              │ (Approver approves)   │ (Admin confirms)
              │                       │
              └──────────┬────────────┘
                         ▼
                   [ACTIVE]
...
4. **Integration Map** — Approval workflows integrate with Authorization, Audit, Configuration

**Next:** EP-07 Compliance (separate document)
