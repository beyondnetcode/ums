...
### R-23: Seeder and Mock Data Sanitation Over Patching
*   **Scope**: `architect`, `dev`, `qa`, `analyst`
*   **Trigger Condition**: Detection of incoherent seed data, broken mocks, inconsistent local datasets, invalid references, missing tenant parameters, incomplete roles or permissions, duplicated records, or repetitive failures caused primarily by initial data rather than business logic.
*   **Instruction**: When the dominant root cause is data incoherence, agents must not apply isolated patches to individual records just to make a single test pass. The affected seeder, mock set, or test dataset must be evaluated holistically and, when necessary, replaced or rewritten end-to-end with coherent, complete, traceable data aligned to the current domain model. The rebuilt dataset must respect tenants, users, user types, roles, profiles, permissions, systems, modules, menus, resources, actions, global parameters, tenant parameters, auth graph dependencies, and audit expectations where applicable. Agents must validate the reconstructed data with the relevant unit, integration, and end-to-end tests, and must update documentation if the seed model or dataset strategy changed.
