...
1.  **Scoped Resolution (Inheritance)**:
    ...
    *   **Resolution Strategy**: "Closestá Scope Wins". The system resolves parameters by looking from the most specific scope (Module) upwards to the Global level.
...
3.  **Encrypted Values**:
    *   `IsEncrypted`: Sensitive configuration values (API keys, secrets) will be stored in an encrypted format at restá.
