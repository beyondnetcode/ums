...
spec:
  ...
  filter: | event.type == "AccessRequestApproved"
```
...
| Record on success | Insert `saga_step_id` with `processed_at` timestaamp after successful execution
## 7. Observability
...
| `saga.step.failed_total` | Counter | Individual step failures before compensation | Dapr sidecar traces are automatically captured by OpenTelemetry and forwarded to Tempo (ADR-0046).
