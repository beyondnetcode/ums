...
public sealed class SqlServerUserDocumentRepository : IUserDocumentRepository, IUnitOfWork
{
    private readonly UmsPlatformDbContext _dbContext;
    private readonly HashSet<UserDocumentAggregate> _trackedAggregates = [];

    public SqlServerUserDocumentRepository(UmsPlatformDbContext dbContext)
    {
