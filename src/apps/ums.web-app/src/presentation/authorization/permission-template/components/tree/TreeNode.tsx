...
export function computeEffectiveState(node: UITreeNodeData): 'Allow' | 'Deny' | 'Neutral' | 'Partial' {
...
export const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const StateIcon = 
    state === 'Allow' ? CheckCircle2 :
    state === 'Deny' ? XCircle :
    state === 'Partial' ? CheckCircle2 :
    MinusCircle;

  const stateColor = 
    state === 'Allow' ? 'text-emerald-500' :
    state === 'Deny' ? 'text-rose-500' :
    state === 'Partial' ? 'text-amber-500' :
