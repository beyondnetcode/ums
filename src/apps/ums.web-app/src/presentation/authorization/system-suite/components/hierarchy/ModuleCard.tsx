...
export const ModuleCard: React.FC<ModuleCardProps> = ({
  ...
  const handleAddMenu = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!menu.code.trim()) {
      setMenu(s => ({ ...s, error: 'Código requerido' }));
      return;
    }
    if (!menu.label.trim()) {
      setMenu(s => ({ ...s, error: 'Etiqueta requerida' }));
      return;
    }
    try {
  ...
  return (
    <div className="border border-m3-outline/20 rounded-lg overflow-hidden bg-m3-surface-container/5 transition-all hover:border-m3-outline/40 animate-fadeIn">
      ...
      <div className={`p-4 space-y-4 ${isExpanded ? 'block' : 'hidden'}`}>
        {module.description && (
          <p className="text-xs text-m3-secondary italic pl-1 border-l-2 border-m3-outline/20">
            {module.description}
          </p>
        )}
        ...
        <InlineAddForm
          isOpen={isAddingMenu}
          onToggle={open => {
            setIsAddingMenu(open);
            if (!open) setMenu(emptyMenu());
          }}
        ...
        >
          <M3TextField
            ...
            required
            value={menu.code}
            onChange={e => setMenu(s => ({ ...s, code: e.target.value }))}
          ...
          <M3TextField
            ...
            required
            value={menu.label}
            onChange={e => setMenu(s => ({ ...s, label: e.target.value }))}
            placeholder="e.g. Administración"
          />
          <M3TextField
            label="Descripción"
            value={menu.desc}
            onChange={e => setMenu(s => ({ ...s, desc: e.target.value }))}
          ...
          <M3TextField
            ...
            type="number"
            value={menu.sort}
            onChange={e => setMenu(s => ({ ...s, sort: e.target.value }))}
            placeholder="1"
          />
        </InlineAddForm>

        {!module.menus || module.menus.length === 0 ? (
          <p className="text-xs text-m3-secondary/40 italic text-center py-4">
