...
export const BrandingPanel: React.FC<BrandingPanelProps> = ({ tenantId, isRootTenant }) => {
  ...
  const handleUpdateBranding = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification({ title: t.notifBrandingApplied, message: t.notifBrandingMsg(fields.primaryColor), type: 'success' });
      await loadBranding();
    } catch {
  ...
  const dnsLabel = dns === 'Verified' ? t.brandDnsVerified : dns === 'Failed' ? t.brandDnsFailed : t.brandDnsPending;
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        ...
        actions={
          <Tooltip content={t.brandDnsStatus}>
            <span>
              <StatusBadge status={dns} label={dnsLabel} colorMap={DNS_COLOR_MAP} className="cursor-default" />
            </span>
          </Tooltip>
        }
      />
      ...
      <form onSubmit={handleUpdateBranding}>
        <p className="text-[11px] font-semibold uppercase tracking-wide text-m3-secondary/70 mb-2">{t.brandingContent}</p>

        <M3TextField compact label={t.brandHeadline} value={fields.headlineText} onChange={(e) => setField('headlineText', e.target.value)} placeholder="e.g. Welcome to Ransa Portal" />
        <M3TextField compact label={t.brandSecondary} value={fields.secondaryText} onChange={(e) => setField('secondaryText', e.target.value)} placeholder="e.g. Sign in to continue" />
        <M3TextField compact label={t.brandButtonLabel} value={fields.primaryButtonLabel} onChange={(e) => setField('primaryButtonLabel', e.target.value)} placeholder="e.g. Sign in" />
        <M3TextField compact label={t.brandFooter} value={fields.footerText} onChange={(e) => setField('footerText', e.target.value)} placeholder="e.g. © 2025 Ransa Perú S.A." />

        <p className="text-[11px] font-semibold uppercase tracking-wide text-m3-secondary/70 mb-2 mt-4">{t.brandingVisual}</p>
        ...
        <div className="flex gap-2 items-start mb-3">
          <div className="flex-1 min-w-0">
            <M3TextField compact label={t.brandLogoUrl} value={fields.logo} onChange={(e) => setField('logo', e.target.value)} placeholder="https://cdn.example.com/logo.png" className="mb-0" />
          </div>
        ...
        <p className="text-[11px] font-semibold uppercase tracking-wide text-m3-secondary/70 mb-2 mt-4">{t.brandingDomain}</p>
        ...
        <M3FieldsetWrapper label={t.brandMagicLink} compact className="mb-4">
          <div className="flex items-center justify-between w-full">
            ...
            <M3Switch checked={fields.magicLinkFallbackEnabled} onChange={(v) => setField('magicLinkFallbackEnabled', v)} />
          </div>
        </M3FieldsetWrapper>
