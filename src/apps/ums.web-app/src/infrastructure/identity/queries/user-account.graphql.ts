...
const GET_USER_ACCOUNTS =
  'query UserAccounts($page: Int!, $pageSize: Int!, $search: String, $criteria: String, $status: String, $sortBy: String, $sortOrder: String, $tenantId: UUID) { userAccounts(page: $page, pageSize: $pageSize, search: $search, criteria: $criteria, status: $status, sortBy: $sortBy, sortOrder: $sortOrder, tenantId: $tenantId) { items { userAccountId tenantId branchId email category status identityReference identityReferenceType hasActivePassword passwordUpdatedAtUtc } page pageSize totalItems totalPages } }';

const GET_USER_ACCOUNT_BY_ID =
  'query UserAccount($userAccountId: UUID!) { userAccountById(userAccountId: $userAccountId) { userAccountId tenantId branchId email category status identityReference identityReferenceType hasActivePassword passwordUpdatedAtUtc } }';
...
export interface GetUserAccountsResponse {
  userAccounts: GraphqlUserAccountPage;
}

export interface GetUserAccountByIdResponse {
  userAccountById: GraphqlUserAccountDto | null;
}
...
export const graphqlQueries = {
  ...
  getUserAccountById: async (userAccountId: string): Promise<GetUserAccountByIdResponse> => {
    if (!userAccountId || userAccountId.trim() === '') {
      throw new GraphQlValidationError('Invalid userAccountId parameter', [
        'userAccountId must be a non-empty string',
      ]);
    }
    return graphqlClient.request<GetUserAccountByIdResponse>(GET_USER_ACCOUNT_BY_ID, {
      userAccountId,
    });
  },
};
