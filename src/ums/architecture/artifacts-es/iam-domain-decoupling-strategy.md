# 🏛️ Estrategia de Desacoplamiento de Dominio: UMS vs Enterprise Suite
...
- **Documentación de Referencia**: Las especificaciones técnicas (`enterprise-iam-ums-specification.md`) utilizan la "Enterprise Suite" no solo como ejemplo, sino como el modelo de requisitos base. El flujo de usuario está centrado en "Analistas de Negocio" y "Sucursales de Terminal".
...
3. **Violación de Clean Architecture**: El círculo del Dominio (Pure POCOs) está permeado por requerimientos de integración externa (ERP/CRM).
...
## 3. 🏗️ Arquitectura Objetivo (Target Architecture)
...
| **Naming** | Route_Planner | **Client_System_ID** |

### 3.2. Refactorización del Dominio (DDD)
...
## 4. 🛠️ Recomendaciones de Enterprise Architect
...
- Implementar el patrón **Property Bag** o campos **JSONB** (PostgreSQL) para extender entidades de IAM con datos específicos de negocio (como el `ErpCode`) sin modificar el esquema relacional estático.
...
- **Domain Logic in Guards**: Realizar validaciones de negocio (ej: "solo si el camión está vacío") dentro del motor de UMS. (Correcto: El UMS valida permisos; la lógica de negocio reside en el microservicio de destino).
...
---
**Conclusión:** El UMS debe ser el cimiento invisible sobre el cual se construyen las suites de negocio, no una extensión de una de ellas. El éxito del escalamiento multi-tenant depende de la pureza y abstracción de este kernel de identidad.
