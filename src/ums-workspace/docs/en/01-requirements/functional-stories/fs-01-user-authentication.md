...
| **Primary Actor** | Corporate User |
| **Preconditions** | User is registered in the ULPMS database and holds a valid HR employee reference. |
| **Postconditions** | Session is established in the client application, and a secure HTTP-Only cookie is returned. |
...
### A. Main Flow
1.  The user accesses the client portal and clicks the "Login with Corporate SSO" button.
...
4.  The IdP redirects the browser back to the client portal with an authorized single-use Authorization Code.
5.  The client backend exchanges the Authorization Code with the IdP for a cryptographically signed Access Token (JWT) containing identity claims.
6.  The backend verifies the token's RS256 signature and validates that the `identity_reference` matches an active identity record in the local client database.
...
*   If the connection to Keycloak/Azure AD fails, the API Gateway intercepts the timeout error.
...
*   If the authenticated IdP token succeeds but the `identity_reference` is not found or is suspended in the client database:
    *   The backend aborts the login process.
    *   Saves a security warning inside the immutable access audit logs.
    *   Returns a `403 Forbidden` response explaining that the corporate account is not active on the client portal.

---

## 📋 4. Primary Operational Model Reference
The complete transaction flow, multi-factor authentication considerations, and error paths for this use case are modeled around the **Business Analyst** initiating a session at the Callao Port Terminal (under *Logistics Corp*). For the detailed technical schemas, parameter structures, and OpenAPI examples, consult **[enterprise-iam-ums-specification.md](../../04-artifacts/enterprise-iam-ums-specification.md)**.
