...
| [`DS-04`](./ums-gap-reference-catalog.md#ds-04) | Inbox de dedup no cableado en el consumidor de proyección: se llama `AddEntityFrameworkOutbox<TenantProjectionDbContext>()` a nivel de bus pero la consumer definition nunca llama `UseEntityFrameworkOutbox<TenantProjectionDbContext>(context)`; `InboxState` existe pero nunca se consulta. | `Messaging` | Release | P1 | S | `PENDING` |
| [`DS-05`](./ums-gap-reference-catalog.md#ds-05) | Carrera read-check-write en el upsert versionado (regresión permanente): sin token de concurrencia, dos eventos en vuelo del mismo tenant pueden regresar la proyección. Requiere escritura condicional set-based `ON CONFLICT … DO UPDATE … WHERE version < EXCLUDED.version`. | `Messaging` | Release | P1 | S | `PENDING` |
| [`DS-07`](./ums-gap-reference-catalog.md#ds-07) | Fallback `Default`/`DefaultConnection` → el contexto de proyección apunta a localhost en silencio cuando falta `MasterDataDb`; el DI cae a un connection string por defecto. Requiere corregir el fallback + `ConnectionStrings__MasterDataDb` explícito en el chart. | `Infra` | Release | P1 | S | `PENDING` |
...
**Progress:** 0 / 7 done · 0 in progress · 7 pending · 0 blocked · 0 deferred
