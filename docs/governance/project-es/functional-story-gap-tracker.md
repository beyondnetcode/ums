...
|---|---|---|---|---|---|
| 1 | **Resolver jerarquico de 4 niveles** — `InMemoryConfigurationCache.GetWithPrecedence()` solo cascadea Tenant → Global. Los alcances Suite y Module son ignorados. | 🔴 P1 / Alta | CA-3, RN-1 | Reemplazar la logica de 2 niveles con una cadena completa: Module → Suite → Tenant → Global. Agregar `GetWithPrecedence(code, tenantId, suiteId, moduleId)`. | `Ums.Infrastructure/Configuration/InMemoryConfigurationCache.cs` |
| 2 | **Flag no-sobrescribible** — No existe la propiedad `IsNonOverridable` en ningun lugar. Los alcances inferiores siempre pueden sobreescribir, violando RN-2 y CA-2. | 🔴 P1 / Alta | CA-2, RN-2 | Agregar `IsNonOverridable` a `AppConfigurationProps` y `ParameterDefinitionProps`. Aplicar un guard en `CreateAppConfigurationCommandHandler` que verifique las configs del alcance padre antes de guardar. | `Ums.Domain/Configuration/AppConfiguration/`, `Ums.Application/Configuration/AppConfiguration/Commands/` |
| 3 | **Alcances Suite y Module no procesados** — `ConfigurationScope.Suite` y `.Module` estan definidos pero ningun handler ni resolver los procesa. Un valor guardado en alcance Suite es efectivamente invisible. | 🔴 P1 / Alta | CA-1, CA-3 | Conectar el filtrado por alcance Suite y Module en `CreateAppConfigurationCommandHandler` y `GetWithPrecedence`. Agregar consultas BD con conciencia de alcance en el repositorio. | `Ums.Application/Configuration/AppConfiguration/Commands/CreateAppConfigurationCommandHandler.cs`, `Ums.Infrastructure/Persistence/Configuration/` |
| 4 | **Proteccion de valores sensibles** — El flag `IsEncrypted` se almacena pero nunca se usa. El valor crudo siempre se retorna en el DTO sin importar el flag ni el rol del llamante. | 🔴 P1 / Alta | RN-5 | Implementar cifrado AES en escritura en el command handler. Redactar `Value` en `AppConfigurationDto` cuando `IsEncrypted=true` y el llamante no es Administrador Global. Agregar verificacion de autorizacion en el query handler. | `Ums.Application/Configuration/AppConfiguration/DTOs/AppConfigurationDto.cs`, `Ums.Application/Configuration/AppConfiguration/Commands/CreateAppConfigurationCommandHandler.cs` |
| 5 | **Endpoint `/resolve` faltante para AppConfiguration** — Los clientes no tienen forma de solicitar el valor efectivo resuelto para una cadena de alcance. El endpoint equivalente existe para `IdpConfiguration` pero no para `AppConfiguration`. | 🟠 P2 / Media | CA-3 | Agregar `GET /app-configurations/resolve?code=X&tenantId=Y&suiteId=Z&moduleId=W`. Conectarlo al resolver jerarquico de la brecha #1. | `Ums.Presentation/Endpoints/Configuration/AppConfiguration/AppConfigurationEndpoints.cs` |
| 6 | **Sin endpoints REST para Parameters** — `ParameterDefinition`, `ParameterGlobalValue` y `ParameterTenantValue` tienen handlers de aplicacion pero sin superficie HTTP ni UI en el frontend. | 🟠 P2 / Media | CA-1 | Agregar endpoints `POST/PUT/GET/DELETE /parameters` y `/parameters/{id}/values`. Agregar un panel de gestion basico en el frontend. | `Ums.Presentation/Endpoints/Configuration/`, `ums.web-app/src/presentation/configuration/` |
