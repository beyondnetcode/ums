...
- **The "Cache Stampede" Event:** A global permissions update forces the expiration of cached ACLs (Access Control Lists) for 50,000 concurrent users. The system must prevent a thundering herd on the SQL Server.
...
- **Objective:** Identify the breaking point of SQL Server RLS policies under heavy locking.
...
- **Execution:** Randomly terminating Redis nodes (causing Cache Stampede) and forcing SQL Server failover while 10,000 requests are mid-flight.
...
## 4. Pending Actions
- Provision a Staging environment mirroring Production resources (Redis Cluster, SQL Server HA Replica).
