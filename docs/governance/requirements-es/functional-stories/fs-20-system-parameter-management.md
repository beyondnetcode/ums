...
UMS debe proporcionar un mecanismo seguro, configurable y auditable para gestionar parámetros a nivel de sistema y específicos de tenant. Los administradores de plataforma internos pueden gestionar parámetros globales y específicos de tenant, mientras que los administradores de tenant solo pueden gestionar parámetros dentro de su propio scope de tenant. Todos los cambios de parámetros deben ser rastreados para cumplimiento y depuración.

## 2. Actores

| Actor | Responsabilidad |
| :--- | :--- |
| **Administrador de Plataforma Interno** | Gestiona parámetros globales del sistema y parámetros específicos de cualquier tenant. Tiene visibilidad completa de configuración. |
| **Administrador de Tenant** | Gestiona parámetros dentro de su propio scope de tenant únicamente. No puede acceder a configuraciones globales ni de otros tenants. |
| **Sistema** | Consume los parámetros configurados para determinar comportamiento, reglas, límites y permisos. |

## 3. Precondiciones de Negocio

- El administrador está autenticado y tiene una sesión válida.
...
- Para admins internos, `isInternalAdmin=true` en los claims del JWT.
...
1. El administrador interno navega a la sección de Configuración del Sistema.
2. UMS valida que `IsInternalAdmin=true` y el usuario tiene `CAN_MANAGE_GLOBAL_CONFIGURATION`.
...
2. Para admins internos: UMS permite ver/gestionar cualquier parámetro de cualquier tenant.
3. Para admins de tenant: UMS valida que `OrganizationId == targetTenantId`.
...
Si un administrador de tenant intenta acceder a configuración global, UMS devuelve un error 403 con mensaje "La configuración global solo es accesible para administradores internos."
...
1. **Scope de Configuración Global**: Parámetros sin `TenantId`, sin `SystemSuiteId` y sin `ModuleId` son Globales. Solo admins internos pueden gestionarlos.
2. **Scope de Configuración de Tenant**: Parámetros con `TenantId` son específicos de tenant. Tanto admins internos como de tenant pueden gestionarlos (admins limitados a su tenant).
...
1. Los administradores internos pueden ver y gestionar todos los parámetros globales.
...
5. Los administradores internos pueden acceder a la configuración de cualquier tenant.
...
| POST | `/app-configurations` | Crear nueva configuración | Admin interno o admin de tenant para su propio tenant |
...
|-----------|----------|
| `scope=Global` | Solo admins internos pueden consultar; retorna todas las configs globales |
| `scope=Tenant&tenantId={id}` | Admins internos obtienen todas las configs de tenant; admins de tenant obtienen solo su tenant |
