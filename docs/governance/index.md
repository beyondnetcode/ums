# Governance Portal

Welcome to the **Governance Portal** for the User Management System (UMS). This section contains the product direction, business scope, functional requirements, and construction-phase engineering design.

## Navigation Hub

| Section | Description |
| :--- | :--- |
| **[Product](./product/index.md)** | Product vision, business context, scope, and objectives. |
| **[Requirements](./requirements/index.md)** | Functional stories, ubiquitous language, and conceptual data model. |
| **[Project Backlog](./project/index.md)** | MVP backlog, epic designs (Approvals, Compliance, IGA). |
| **[Construction](./construction/index.md)** | DDD domain layer design, technical stories, implementation plans. |

---

**[Back to Master Index](../MASTER_INDEX.md)** | **[Back to Root README](../../README.md)**
